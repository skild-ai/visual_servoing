...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    #! for camera mounta
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT94VWMV-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2),
        joint_offsets=(
            2*np.pi/2, 
            2*np.pi/2, 
...
NAME_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    "tv": DynamixelRobotConfig(
        joint_ids=(1, 2),
        joint_offsets=(
            2*np.pi/2, 
            2*np.pi/2, 
...
class DynamixelAgent(Agent):
    def __init__(
        ...
        # find the info auto
            ...
            assert name in NAME_CONFIG_MAP, f"Name {name} not in config map"

            # use port to gain config
            config = NAME_CONFIG_MAP[name]
